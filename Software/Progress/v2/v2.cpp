...
class menu
{
	public:
		void displaymenu()
		{
			...
			settextstyle(3,0,int(6*(maxx+200)/959));
			strcpy(str,"3D RENDERING SOFTWARE");
			outtextxy(maxx/6,maxy-400,str);
			
			settextstyle(3,0,int(4*(maxx+200)/959));
			strcpy(str,"BY PRATUL VENKATESH");
			outtextxy(maxx/6,maxy-305,str);
		...
		void displayinstructions()
		{
			...
			int lspacing=maxy-400;
			char str[80];
			settextstyle(3,0,int(5*(maxx+200)/959));
			f.getline(str,80,'\n');
			outtextxy(maxx/6,lspacing,str);
			
			lspacing+=40;
			settextstyle(3,0,int(3*(maxx+200)/959));
...
/*	cout<<"Save File?";
    ...
    float pva=-pi/7; //vertical factor
    calccampos(pha,pva);
